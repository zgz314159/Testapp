...
class FavoriteQuestionRepositoryImpl @Inject constructor(
    ...
    private companion object {
        // companion kept for future local defaults if needed
    }
    ...
    override suspend fun importFromFile(file: java.io.File): Int {
        return try {
            ...
            if (jsonFavorites != null) {
                ...
                return jsonFavorites.size
            }

            // 否则尝试解析 Excel
            val excelData = parseExcelFavorites(file)
            excelData.forEach { (favorite, aiTriple) ->
                dao.add(favorite.toEntity())
                aiTriple?.let { (deepSeek, spark, baidu) ->
                    try {
    ...
    suspend fun exportFavoritesToExcel(
        ...
        return try {
            ...
            val grouped = favorites.groupBy { it.question.fileName ?: ExportConstants.DEFAULT_GROUP_NAME }
    ...
    suspend fun prepareExportSheetsForFavorites(
        favorites: List<FavoriteQuestion>,
        headers: List<String>,
    ...
    override suspend fun exportToFile(file: java.io.File): Boolean {
        ...
        return exportFavoritesToExcel(favorites, file)
    }
