...
interface HistoryRecordDao {
    @Query("SELECT * FROM history_records ORDER BY time DESC, id DESC")
    ...
    @Query("SELECT * FROM history_records WHERE fileName = :fileName ORDER BY time DESC, id DESC")
    fun getByFileName(fileName: String): Flow<List<HistoryRecordEntity>>

    @Query("SELECT * FROM history_records WHERE fileName IN (:fileNames) ORDER BY time DESC, id DESC")
