...
- 启动碎化放到 `Dispatchers.Default`；关闭碎化热路径调试日志。

## 2026-07-31 | 磁吸「细碎/原子级」档位语义对调并增强

- 原「原子级」（有分值吸附）改名为「细碎」（storage=5）。
- 最右「原子级」（storage=6）改为题库原样细碎：空与夹缝均可拖，仅标点归属，观感对齐原细碎截图且与原子题库一致。

## 2026-07-31 | 磁吸「原子级」设置落盘竞态修复

- 碎化滑杆拖动时改为只预览；松手后才写入 DataStore，并用 NonCancellable 防止离开设置页取消落盘。
- 避免 UI 已显示「原子级」但会话仍读到中间档（如「大块」）导致词块异常变粗。
- 碎化刷新提示中显式带出当前档位名，便于确认生效。

## 2026-07-31 | 磁吸「原子级」0分词吸附完善

- 滑杆第六档「原子级」沿用题库有分值原子为词块；`【0分】` 现可正确解析为 score=0。
- 0 分原子与夹缝连接词（的/和/及/并/与…）及标点不得单独成块，吸附到相邻有分值词块。
- 设置文案同步标明「原子级」与 0 分吸附规则。

## 2026-07-31 | 主页左侧抽屉反复弹出/收回修复

- 现象：打开主页左侧题库抽屉后点遮罩收起，随即又自动弹出。
- 日志定位：`CONFIRM Closed`（Scrim 点击）→ 关闭动画完成 `cur=Closed` → 约 25ms 后 `CONFIRM Open`，堆栈为 `ModalNavigationDrawer.measure` → `updateAnchors` → `trySnapTo`（非业务 `open()`）。
- 根因：关闭后卸载 `drawerContent` 使抽屉测量宽度归零，anchors 重算把状态 snap 回 Open。
- 修复：首次打开前用同宽占位稳住 anchors；打开过后保持抽屉内容挂载，关闭不再卸载。移除易冲突的自定义横滑关闭。

## 2026-07-29 | 磁吸重建增加词块碎化程度设置

- 设置 → 出题模式 → 磁吸重建新增五档离散调节：大块、偏粗、标准、偏细、细碎。
- 磁吸 Pipeline 根据粒度将原子片段合并为最多 6/9/12/16/24 个词块，并优先保护句号、逗号等语义边界。
- 未完成磁吸轮次保存并继续使用原粒度，避免设置变化破坏当前题号和词块布局；完成本轮或主页“重答”后应用新粒度。
- 该设置独立于普通填空题、自适应渐隐、练习和考试。
