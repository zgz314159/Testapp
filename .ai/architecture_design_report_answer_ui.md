...
- Practice/Exam/Adaptive 复用 `:ui-common` 的答题视觉 token，浅色页面保持护眼灰，主卡 `#F8F7F4`、选项 `#FDFCF9`、软边框 `#E9E6DF`。
- 卡片使用显式 `shadow(clip=false) + background + border`，不得依赖 Material3 默认 container/elevation token，也不得在外层 `Surface` 裁切阴影。
