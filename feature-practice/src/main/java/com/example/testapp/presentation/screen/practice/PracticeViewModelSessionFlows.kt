...
internal class PracticeViewModelSessionFlows private constructor(
    ...
    val uiQuestions: StateFlow<List<QuestionUiModel>> = sessionState.map { state ->
    ...
    val sessionAnsweredCountFlow: StateFlow<Int> = sessionState.map { it.sessionAnsweredCount }
        .distinctUntilChanged()
        .stateIn(scope, SharingStarted.Lazily, 0)

    val sessionCorrectCountFlow: StateFlow<Int> = sessionState.map { it.sessionCorrectCount }
        .distinctUntilChanged()
        .stateIn(scope, SharingStarted.Lazily, 0)

    val hasAnyInputInSessionFlow: StateFlow<Boolean> = sessionState
        .map { PracticeSessionInputPipeline.hasAnyInput(it.questionsWithState) }
        .distinctUntilChanged()
        .stateIn(scope, SharingStarted.Lazily, false)

    val sessionInputCountFlow: StateFlow<Int> = sessionState
        .map { PracticeSessionInputPipeline.inputCount(it.questionsWithState) }
        .distinctUntilChanged()
        .stateIn(scope, SharingStarted.Lazily, 0)

    val currentQuestionUi: StateFlow<PracticeCurrentQuestionUi?> = sessionState
        .map { PracticeCurrentQuestionUiPipeline.snapshot(it) }
        .distinctUntilChanged()
        .stateIn(scope, SharingStarted.Lazily, null)

    val answeredList: StateFlow<List<Int>> = sessionState.map { it.answeredIndices }
        .stateIn(scope, SharingStarted.Lazily, emptyList())

    val selectedOptions: StateFlow<List<List<Int>>> = sessionState.map { state ->
    ...
    val showResultList: StateFlow<List<Boolean>> = sessionState.map { state ->
        state.questionsWithState.map { it.showResult }
    }.stateIn(scope, SharingStarted.Lazily, emptyList())

    val analysisList: StateFlow<List<String>> = sessionState.map { state ->
        state.questionsWithState.map { it.analysis }
    }.stateIn(scope, SharingStarted.Lazily, emptyList())

    val sparkAnalysisList: StateFlow<List<String>> = sessionState.map { state ->
        state.questionsWithState.map { it.sparkAnalysis }
    }.stateIn(scope, SharingStarted.Lazily, emptyList())

    val baiduAnalysisList: StateFlow<List<String>> = sessionState.map { state ->
        state.questionsWithState.map { it.baiduAnalysis }
    }.stateIn(scope, SharingStarted.Lazily, emptyList())

    val noteList: StateFlow<List<String>> = sessionState.map { state ->
        state.questionsWithState.map { it.note }
    }.stateIn(scope, SharingStarted.Lazily, emptyList())

    val textAnswers: StateFlow<List<String>> = sessionState.map { state ->
        state.questionsWithState.map { it.textAnswer }
    }.stateIn(scope, SharingStarted.Lazily, emptyList())

    companion object {
        fun create(sessionState: StateFlow<PracticeSessionState>, scope: CoroutineScope) =
            PracticeViewModelSessionFlows(sessionState, scope)
    }
}
