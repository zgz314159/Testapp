...
@Database(
    ...
    version = 24,  // 升级版本以支持ExamHistoryRecordEntity
    exportSchema = false
)
@TypeConverters(
    IntListConverter::class, 
    BooleanListConverter::class, 
    NestedIntListConverter::class, 
    StringListConverter::class,
    QuestionAnswerStateMapConverter::class,
    ExamQuestionStateMapConverter::class
)
abstract class AppDatabase : RoomDatabase() {
    abstract fun questionDao(): QuestionDao
    abstract fun wrongQuestionDao(): WrongQuestionDao
    abstract fun historyRecordDao(): HistoryRecordDao
    abstract fun examHistoryRecordDao(): ExamHistoryRecordDao  // 重新启用考试历史记录DAO
