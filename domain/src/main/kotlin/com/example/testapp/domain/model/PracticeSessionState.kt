...
/**
 * PracticeSessionState → UnifiedSessionState 转换
 */
fun PracticeSessionState.toUnifiedSessionState(): UnifiedSessionState = UnifiedSessionState(
...
/**
 * 在 PracticeSessionState 中按 index 更新单个 QuestionWithState
 */
fun PracticeSessionState.updateAt(index: Int, transform: (QuestionWithState) -> QuestionWithState): PracticeSessionState =
    if (index !in questionsWithState.indices) this
    else copy(questionsWithState = questionsWithState.mapIndexed { i, qws -> if (i == index) transform(qws) else qws })
