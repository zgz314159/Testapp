...
private fun answerLettersToIndices(answer: String): List<Int> {
...
private fun normalizeFillAnswer(text: String): String {
...
private fun parseFillAnswerPartDescriptor(rawPart: String): FillAnswerPartDescriptor {
...
private fun splitFillAnswerDescriptors(answerText: String): List<FillAnswerPartDescriptor> {
    return splitFillAnswerParts(answerText).map(::parseFillAnswerPartDescriptor)
}

private fun resolveFillCorrectAnswerParts(question: Question): List<String> {
...
fun isFillAnswerCorrect(userAnswer: String, correctAnswer: String): Boolean {
    ...
    if (correctAnswer.contains(FILL_PART_DELIMITER) || userAnswer.contains(FILL_PART_DELIMITER)) {
        val userParts = userAnswer.split(FILL_PART_DELIMITER).map { normalizeFillAnswer(it) }
        val correctParts = splitFillAnswerDescriptors(correctAnswer).map { normalizeFillAnswer(it.answerText) }.filter { it.isNotBlank() }
        if (userParts.size != correctParts.size || correctParts.isEmpty()) return false
        return userParts.zip(correctParts).all { (user, correct) -> user.isNotBlank() && user == correct }
    }
    val normalizedUser = normalizeFillAnswer(userAnswer)
    if (normalizedUser.isBlank()) return false
    val acceptedAnswers = splitFillAnswerDescriptors(correctAnswer).map { normalizeFillAnswer(it.answerText) }.filter { it.isNotBlank() }
